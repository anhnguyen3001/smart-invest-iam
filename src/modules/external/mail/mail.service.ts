...
export class MailService {
  ...
  async sendForgetPasswordMail(email: string, token: string) {
    ...
    await this.mailerService.sendMail({
      ...
      html: `<div style="width: 640px; text-align: center; margin: auto; padding: 10px 20px 25px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;"><h1 style="margin: 0; font-size: 30px;">Welcome to AH Ticker</h1><p style="margin: 24px 0 30px; text-align: left;">We got a request to reset your password. To complete the process, please click this button below.</p><a href="${url}"><button style=" background-color: #000A19; color: #fff; border: none; padding: 16px 25px; font-size: 16px; cursor: pointer; border-radius: 4px;">Reset password</button></a><p style="margin: 30px 0 0; text-align: left;">Welcome to AH Ticker Community!</p><p style=" margin: 6px 0 0; text-align: left; font-size: 13px;">The AH Ticker Team</p></div>`,
    });
  }
}
